#' Calculates the maximum likelihood estimate and the value of the likelihood at the MLE of a chromosome or ploidy model (wrapper function of nloptr for chromploid models)
#' For now it only works for bichrom and bichrom reduced model
#'
...
#' @param model.args List of arguments passed in each specified model other than parameters
...
chromploid_optim<-function(phy.tree, tip.values, model="bichrom",model.args=list(size=50),starting.val, root.dist, optim.options=list("algorithm"= "NLOPT_LN_SBPLX","ftol_rel"=1e-08,"print_level"=1,"maxtime"=170000000, "maxeval"=1000)){
	if(model=="bichrom"){
	results<-rep(0,12)
